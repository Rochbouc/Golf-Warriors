...
function getAppUrl(){
  ...
  // Auto-detect: use current URL unless it's a local file
...
/* ── AVATAR COMPONENT ── */
function Av({player,size=28,style={}}){
  const bg=avColor(player?.name||player?.email||'x');
  const ini=initials(player?.name||player?.email||'?');
  return(
    <div className="av" style={{width:size,height:size,background:player?.photo?'transparent':bg,fontSize:size*.3,...style}}>
...
function LoginScreen({onLogin}){
  ...
  const doLogin=()=>{
    ...
    if(!user){setErr('This email is not registered. Contact the admin to get access.');return;}
    ...
    <div className="login-wrap">
      <div className="login-card">
        ...
        <div className="login-invited">
          🔒 <strong>Invite only.</strong> You must be added by the admin before you can sign in.
        </div>
        ...
        <div className="login-form">
          ...
          <button className="btn-p" style={{width:'100%',padding:'.75rem',marginTop:'.25rem'}} onClick={doLogin}>
            Sign In →
          </button>
          <div className="login-note">
            Default password for all players: <strong>golf</strong><br/>
            You can change it after logging in under your profile.
          </div>
        </div>
      </div>
    </div>
...
function ProfilePage({currentUser,onUpdate,toast}){
  ...
  const handlePhoto=e=>{
    ...
    reader.onload=ev=>{
      ...
      savePlayers(updated);
      onUpdate({...currentUser,photo:ev.target.result});
  ...
  const save=()=>{
    ...
    const updatedUser={...currentUser,name:name.trim(),email:email.trim().toLowerCase(),...(pass?{password:pass}:{})};
    const updated=players.map(p=>p.id===currentUser.id?updatedUser:p);
    savePlayers(updated);
  ...
  return(
    <div className="stats">
      {[{lbl:'Upcoming',val:upcoming,sub:'rounds'},{lbl:'Total',val:teeTimes.length,sub:'all time'},
        ...
        .map(s=><div className="sc" key={s.lbl}><div className="sc-accent"/><div className="sc-lbl">{s.lbl}</div><div className="sc-val">{s.val}</div><div className="sc-sub">{s.sub}</div></div>)}
    </div>
  );
}
...
function TeeCard({tee,players,currentUser,onOpen,onDelete,isAdmin}){
  ...
  const myBadge=myStatus==='yes'
    ?<span style={{fontSize:'.65rem',fontWeight:700,background:'#edf7ee',color:'#276228',padding:'2px 8px',borderRadius:20}}>✓ You're In</span>
    :myStatus==='no'
    ?<span style={{fontSize:'.65rem',fontWeight:700,background:'#fce8e6',color:'#c62828',padding:'2px 8px',borderRadius:20}}>✗ You're Out</span>
    :(tee.invites||[]).includes(currentUser?.id)
    ?<span style={{fontSize:'.65rem',fontWeight:700,background:'#f5f5f5',color:'#888',padding:'2px 8px',borderRadius:20}}>? Pending</span>
    :null;
  return(
    <div className="tc" onClick={()=>onOpen(tee)} style={{cursor:'pointer'}}>
      ...
      <div className="tc-body">
        ...
        <div className="tc-foot">
          ...
          {/* Clickable RSVP status button */}
          <button
            onClick={e=>{e.stopPropagation();onOpen(tee);}}
            style={{
              ...
              borderRadius:8,padding:'.35rem .65rem',cursor:'pointer',transition:'all .15s',
              fontFamily:'inherit',
            }}
            onMouseEnter={e=>{e.currentTarget.style.transform='scale(1.04)';e.currentTarget.style.boxShadow='0 2px 8px rgba(0,0,0,.1)'}}
            onMouseLeave={e=>{e.currentTarget.style.transform='scale(1)';e.currentTarget.style.boxShadow='none'}}
            title="Click to update your status"
          >
            <div style={{display:'flex',gap:'.35rem',fontSize:'.72rem',fontWeight:700}}>
              {yes>=4
                ? <span style={{color:'#c62828',fontWeight:800,fontSize:'.68rem',letterSpacing:'.04em'}}>⛳ FULL 4/4</span>
                : <><span className="ty">✓{yes}</span><span className="tn">✗{no}</span><span className="tp">?{pend}</span></>
              }
            </div>
            <span style={{
              fontSize:'.6rem',fontWeight:700,letterSpacing:'.03em',
              color: myStatus==='yes'?'#276228':myStatus==='no'?'#c62828':'var(--text3)',
            }}>
    ...
    <div className="overlay" onClick={e=>{if(e.target===e.currentTarget)onClose()}}>
      <div className="modal">
        ...
        <div className="modal-body">
          ...
          {live.notes&&<div style={{fontSize:'.78rem',color:'var(--text3)',marginBottom:'1rem',padding:'.5rem .75rem',background:'var(--glass)',borderRadius:'var(--r-sm)',borderLeft:'2px solid var(--accent)'}}>{live.notes}</div>}
            ...
            return(
              <div style={{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:'var(--r-sm)',padding:'.9rem 1rem',marginBottom:'1rem'}}>
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'.6rem'}}>
                  ...
                  <div style={{fontSize:'.7rem',fontWeight:700,color:isFull?'#c62828':'#276228'}}>
                    {yesCount}/4 spots filled
                  </div>
                </div>
                {isFull&&!alreadyIn&&(
                  <div style={{background:'#fce8e6',border:'1px solid #ef9a9a',borderRadius:'var(--r-sm)',padding:'.6rem .85rem',
                    marginBottom:'.75rem',fontSize:'.82rem',color:'#c62828',fontWeight:600,lineHeight:1.5,textAlign:'center'}}>
                    ⛳ This tee time is full! (4/4)<br/>
                    <span style={{fontWeight:400,fontSize:'.78rem'}}>Please check the next available tee time.</span>
                  </div>
                )}
                <div style={{display:'flex',gap:'.5rem'}}>
                  <button onClick={()=>onRsvp(live.id,currentUser.id,'yes')}
                    disabled={isFull&&!alreadyIn}
                    style={{flex:1,padding:'.65rem .5rem',
                      border:`2px solid ${myStatus==='yes'?'#276228':'#e0e0e0'}`,
                      borderRadius:'var(--r-sm)',
                      background:myStatus==='yes'?'#edf7ee':isFull&&!alreadyIn?'#f5f5f5':'#fff',
                      color:myStatus==='yes'?'#276228':isFull&&!alreadyIn?'#bbb':'#555',
                      fontFamily:'Syne,sans-serif',fontSize:'.85rem',fontWeight:700,
                      cursor:isFull&&!alreadyIn?'not-allowed':'pointer',transition:'all .2s',
                      opacity:isFull&&!alreadyIn?.6:1}}>
                    ✅ I'm In
                  </button>
                  <button onClick={()=>onRsvp(live.id,currentUser.id,'no')}
                    style={{flex:1,padding:'.65rem .5rem',
                      border:`2px solid ${myStatus==='no'?'#c62828':'#e0e0e0'}`,
                      borderRadius:'var(--r-sm)',
                      background:myStatus==='no'?'#fce8e6':'#fff',
                      color:myStatus==='no'?'#c62828':'#555',
                      fontFamily:'Syne,sans-serif',fontSize:'.85rem',fontWeight:700,cursor:'pointer',transition:'all .2s'}}>
                    ❌ Can't Make It
                  </button>
                </div>
      ...
      <div className="fcard">
        ...
        <div className="isec">
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'.35rem'}}>
            ...
            <div style={{display:'flex',gap:'.5rem'}}>
              <button onClick={selectAll} style={{background:'none',border:'none',fontSize:'.72rem',color:'var(--text2)',cursor:'pointer',fontFamily:'Syne,sans-serif',fontWeight:600,textDecoration:'underline'}}>All</button>
  ...
  return(
    <div className="page">
      <div className="ph"><div className="ph-left"><div className="ph-eyebrow">Management</div><h2>Players</h2></div>
        <button className="btn-p" onClick={()=>{setShowForm(s=>!s);setName('');setEmail('');setPass('')}}>{showForm?'Cancel':'+ Add Player'}</button>
      </div>
      ...
      <div className="sec-label">All Players — click to edit</div>
      <div className="pgrid">
        {players.filter(p=>p.role!=='admin').map(p=>{
          ...
          return(
            <div className="pc" key={p.id} onClick={()=>setEditPlayer(p)}
              style={{cursor:'pointer',transition:'all .18s'}}
              onMouseEnter={e=>e.currentTarget.style.borderColor='var(--border2)'}
              onMouseLeave={e=>e.currentTarget.style.borderColor='var(--border)'}>
              <div className="pav" style={{background:p.photo?'transparent':avColor(p.name),position:'relative'}}>
                {p.photo?<img src={p.photo} alt={p.name}/>:initials(p.name)}
                <div style={{position:'absolute',inset:0,background:'rgba(0,0,0,.35)',borderRadius:'50%',
                  display:'flex',alignItems:'center',justifyContent:'center',opacity:0,transition:'opacity .15s',fontSize:'.65rem',color:'#fff',
                  fontWeight:700}} className="pav-edit-hint">✏️</div>
              </div>
              <div className="pinfo">
...
function PlayerEditModal({player,players,onSave,onDelete,onClose,toast}){
  ...
  const save=()=>{
    ...
    if(conflict){toast('Email already used by another player.','err');return;}
    onSave({...player,name:name.trim(),email:email.trim().toLowerCase(),photo,...(pass?{password:pass}:{})});
  };

  const confirmDelete=()=>{
    if(confirm(`Remove ${player.name} from Golf Warriors? This cannot be undone.`)) onDelete();
  };
        ...
        <div className="modal-body">
          ...
          <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:'.6rem',marginBottom:'1.5rem'}}>
            <div onClick={()=>fileRef.current.click()}
              style={{width:80,height:80,borderRadius:'50%',overflow:'hidden',border:'2px solid var(--border)',
                display:'flex',alignItems:'center',justifyContent:'center',
                background:photo?'transparent':avColor(player.name),
                cursor:'pointer',position:'relative',flexShrink:0}}>
              {photo
                ?<img src={photo} alt={name} style={{width:'100%',height:'100%',objectFit:'cover'}}/>
                :<span style={{color:'#fff',fontSize:'1.5rem',fontWeight:700}}>{initials(name||player.name)}</span>
              }
              <div style={{position:'absolute',inset:0,background:'rgba(0,0,0,.4)',borderRadius:'50%',
                display:'flex',alignItems:'center',justifyContent:'center',opacity:0,transition:'opacity .15s',fontSize:'1.1rem'}}
                className="photo-hover-overlay">📷</div>
            </div>
            <input ref={fileRef} type="file" accept="image/*" style={{display:'none'}} onChange={handlePhoto}/>
          ...
          {/* Fields */}
          <div style={{display:'flex',flexDirection:'column',gap:.9+'rem'}}>
            <div className="fg">
            ...
            <div className="fg">
              <label>New Password <span style={{fontWeight:400,textTransform:'none',letterSpacing:0,color:'var(--text3)',fontSize:'.65rem'}}>(leave blank to keep current)</span></label>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:'1.5rem',paddingTop:'1.25rem',borderTop:'1px solid var(--border)'}}>
            <button onClick={confirmDelete}
              style={{background:'transparent',border:'1px solid #ef9a9a',color:'var(--red)',padding:'.5rem 1rem',
                borderRadius:'var(--r-sm)',fontFamily:'Syne,sans-serif',fontSize:'.75rem',fontWeight:600,cursor:'pointer',transition:'all .15s'}}
      ...
      <div className="cal-wrap">
        <div className="cal-card">
          <div className="cal-nav">
            <button className="cal-arr" onClick={prevMonth}>‹</button>
            ...
            <button className="cal-arr" onClick={nextMonth}>›</button>
          </div>
          <div className="cal-grid">
            ...
            {cells.map((day,i)=>{
              ...
              return(
                <div key={day} className={`cal-cell${isToday?' today':''}${isSel?' sel':''}${tees.length?' has-tee':''}`}
                  onClick={()=>setSelected(isSel?null:day)}
                  title={tees.length?`${tees.length} tee time${tees.length>1?'s':''}`:canManagePlayers?'Click to book':''}
                >
                  <span className="cal-num">{day}</span>
                  {tees.length>0
                    ?<div className="cal-dots">{tees.slice(0,3).map(t=><div className="cal-dot" key={t.id}/>)}</div>
                    :canManagePlayers&&<div style={{fontSize:'.5rem',color:'var(--text3)',opacity:0}} className="cal-plus">+</div>
                  }
                </div>
              );
            })}
          ...
          <div style={{fontSize:'.68rem',color:'var(--text3)',marginTop:'.75rem',textAlign:'center'}}>
            {canManagePlayers?'Click a day to view or + add a tee time':'Click a day to view tee times'}
          </div>
        </div>
        ...
        <div className="cal-side">
          {selected?(
            <>
              <div className="cal-side-hd">
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                  ...
                  {canManagePlayers&&(
                    <button className="btn-p" style={{padding:'.4rem .85rem',fontSize:'.72rem'}}
                    ...
                    <div className="cal-event" key={t.id} onClick={()=>onOpen(t)}>
                      ...
                      <div className="cal-ev-players">
                        <div style={{display:'flex',marginLeft:5}}>
                          {invPl.slice(0,4).map(p=><Av key={p.id} player={p} size={22}/>)}
                          {invPl.length===0&&<span style={{fontSize:'.68rem',color:'var(--text3)'}}>No invites</span>}
                        </div>
                        <div style={{display:'flex',gap:'.3rem'}}>
                          {canEdit&&<button className="ac-btn" style={{padding:'.28rem .55rem',fontSize:'.65rem'}} onClick={e=>{e.stopPropagation();onEdit(t)}}>✏️ Edit</button>}
                        </div>
                      </div>
                    </div>
                  );
            ...
            <div style={{textAlign:'center',padding:'3rem 1rem',color:'var(--text3)'}}>
              ...
              <div style={{fontSize:'.82rem',lineHeight:1.6}}>Click a day to see tee times.{canManagePlayers&&<><br/>Then tap <strong>+ Book</strong> to add one.</>}</div>
            </div>
          )}
        </div>
      </div>
...
/* ── HISTORY ── */
function History({teeTimes,players,currentUser,onOpen,isAdmin}){
      ...
      <div className="modal" style={{maxWidth:520}}>
        ...
        <div className="modal-body">
          ...
          <div style={{marginBottom:'1.5rem',paddingBottom:'1.5rem',borderBottom:'1px solid var(--border)'}}>
            ...
            <input className="settings-input" type="url" placeholder="https://yourusername.github.io/golf-warriors"
              value={appUrl} onChange={e=>setAppUrl(e.target.value)}/>
            <div className="settings-hint" style={{marginTop:'.4rem'}}>
              The public URL where your app is hosted (e.g. GitHub Pages). This link is included in every invitation email so players can click directly to log in and update their status.
            </div>
            {appUrl&&(
              <div style={{marginTop:'.6rem',background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:6,padding:'.5rem .75rem',fontSize:'.74rem',color:'var(--text2)',wordBreak:'break-all'}}>
                📧 Email link: <strong style={{color:'var(--text)'}}>{appUrl.trim()}</strong>
              </div>
            )}
            {!appUrl&&(
              <div style={{marginTop:'.6rem',background:'#fff8e1',border:'1px solid #ffe082',borderRadius:6,padding:'.5rem .75rem',fontSize:'.74rem',color:'#7c5c00'}}>
                ⚠️ No URL set — invitation emails will not contain a working link to the app.
              </div>
            )}
          </div>
          ...
          <div className="settings-row">
            <div><div className="settings-label">Public Key</div><input className="settings-input" type="text" placeholder="abc123XYZ..." value={pk} onChange={e=>setPk(e.target.value)}/><div className="settings-hint">Account → API Keys in EmailJS dashboard</div></div>
            <div><div className="settings-label">Service ID</div><input className="settings-input" type="text" placeholder="service_xxxxxxx" value={sid} onChange={e=>setSid(e.target.value)}/><div className="settings-hint">Email Services in EmailJS dashboard</div></div>
...
function App(){
  const[currentUser,setCurrentUser]=useState(()=>{
    const s=localStorage.getItem('gw_session');
  ...
  // Handle ?open=TEEID from email link — auto-open that tee time after login
  useEffect(()=>{
  ...
  const handleSaveTee=(tee)=>{
    ...
    setEditTee(null);setTab('dashboard');
  };
  ...
  const handleDelete=id=>{
    ...
    toast('Tee time deleted.');
  };

  const handleAddPlayer=p=>{
    const updated=[...players,p];setPlayers(updated);savePlayers(updated);
  };

  const handleUpdatePlayer=updated=>{
    const u=players.map(p=>p.id===updated.id?updated:p);
    setPlayers(u);savePlayers(u);
    // Also update currentUser if they edited themselves
    if(currentUser.id===updated.id)setCurrentUser(updated);
  };

  const handleDeletePlayer=id=>{
    const u=players.filter(p=>p.id!==id);
    setPlayers(u);savePlayers(u);
  };

  const handleUpdateUser=updated=>{
    setCurrentUser(updated);
    setPlayers(prev=>{const u=prev.map(p=>p.id===updated.id?updated:p);savePlayers(u);return u});
  };
